...
void printIntervalo(arvoreB * r,int a,int b){
    if(r == NULL) return;
    ...
    for(i = 0; i < r->numChaves;i++){
       ...
       if (r->chave[i] > b) {
            printIntervalo(r->filho[i + 1], a, b);
            return;
        }
...
int somaIntervalo(arvoreB * r, int a, int b){
    if(r == NULL) return 0;
    ...
    for(i = 0; i < r->numChaves;i++){
        if(r->chave[i] > a){
            soma += somaIntervalo(r->filho[i],a,b);
        }
        if(r->chave[i] >= a && r->chave[i] <= b){
            soma += r->chave[i];
        }
        if(r->chave[i] > b){
            soma += somaIntervalo(r->filho[i + 1],a,b);
        }
    }
...
int contaFolha(arvoreB * r){
    int i,c=1;
    
    if(r == NULL) return 0;
    
    for(i = 0; i < r->numChaves;i++){
        if(r->filho[i] == NULL) return 1;
        else{
            c += contaFolha(r->filho[i]);
        }
    }
...
int main(){
    ...
    //printf("\n");
    printIntervalo(raiz,1,5);
    //if(buscar(raiz,22)){
    //    printf("Tem na arvoreB\n");
    //}else{
    //    printf("Nao tem na arvoreB\n");
    //}
    printf("%d",somaIntervalo(raiz,1,5));
