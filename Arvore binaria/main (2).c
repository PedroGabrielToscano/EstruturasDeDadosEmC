...
int contaChaves(Arvore r){
    ...
    qtd += contaChaves(r->esq);
    qtd += contaChaves(r->dir);
    
    return qtd;
}

void imprimirIntervalo(Arvore r, int a,int b){
...
int somaIntervalo(Arvore r,int a,int b){
    if(r == NULL) return 0;
    
    if(r->info >= a && r->info <= b){
        return r->info + somaIntervalo(r->esq,a,b) + somaIntervalo(r->dir,a,b);
    }
    if(r->info < a) return somaIntervalo(r->dir,a,b);
    if(r->info > b) return somaIntervalo(r->esq,a,b);
}
...
int main(){
    ...
    raiz = inserirIterativo(raiz, 90);
    raiz = inserirIterativo(raiz, 12);
  ...
  /*  raiz = remover(raiz, 11);
    ...
    printf("Soma Intervalo = %d\n",somaIntervalo(raiz,8,20));
    imprimirIntervalo(raiz,8,20);
